...
export const whenResponse: WhenResponse = curryN(
  ...
  (
    ...
    ifTrueFn: <TOutput = void>(
      response: ResponseSync
    ) => TOutput | Promise<TOutput>
  ) => [
    (response: ResponseSync) =>
      response instanceof Response && conditionFn(response),
    ifTrueFn,
  ]
);
...
export const when200Json: WhenJsonResponse = <TInput, TOutput = void>(
  ...
  whenResponse(
    (r) => both(is200, isJson)(r),
    wrapJsonAsInput<TInput, TOutput>(if200JsonFn)
  );
