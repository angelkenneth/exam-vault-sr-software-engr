...
import { JwtUserPayload, PublicUser } from '@/app/users/_local/user';
import { NextResponse } from 'next/server';
import { toZodError } from '@/lib/shared/to-zod-error';

export const decodeJwt = (secretKey: string, token: string): JwtUserPayload => {
  ...
  try {
    ...
    if (error instanceof JsonWebTokenError) {
      ...
      throw NextResponse.json(toZodError({ message: error.message }), {
        status: 401,
      });
    }
    throw error;
